...
def plot_1D(samples, test_split):
    ...
    X_train = np.vstack((data1[:train_samples], data2[:train_samples], data3[:train_samples], data4[:train_samples]))
    X_test = np.vstack((data1[train_samples:], data2[train_samples:], data3[train_samples:], data4[train_samples:]))

    kmeans.fit(X_train)
    fig, ax = kmeans.plot(X_train, X_test)
